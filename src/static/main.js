  ...
  mutations: {
    load_runids: function(state, payload)
    {
    	...
    	if (state.cursors_by_test[testname] != "-")
    	{
			...
			payload.http.get('/megatest/runs', {params: lquery}).then(
			    function (response) 
			    {
    ...
    {
	    app.$http.post('/megatest/runs', {action: "cancel", id: id}).then(
			    function (response) 
			    {
    ...
    {
	    app.$http.post('/megatest/runs', {action: "delete", id: id}).then(
		    function (response) 
		    {
		    	console.log(response);
		    }
	...
	if (run && ["pass", "fail"].indexOf(run.status) < 0)
	{
		setTimeout(
	    	function wait_for_run() {
	            app.$http.get('/megatest/runs', {params: {id: id}}).then(
				    function (response) 
				    {
				    	console.log(response);
